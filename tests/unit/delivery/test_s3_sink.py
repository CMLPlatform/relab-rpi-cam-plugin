...
from app.delivery.base import ImageSinkError
...
def patch_aioboto3(fake_s3_client: _FakeS3Client) -> Iterator[MagicMock]:
    ...
    with patch("app.delivery.s3_sink.aioboto3") as mock_aioboto3:
...
class TestS3CompatibleSink:
    ...
    async def test_client_is_reused_across_puts(self, patch_aioboto3: MagicMock) -> None:
        """The warm client is created once and reused, not rebuilt per upload."""
        sink = _make_sink()
        for image_id in ("a1b2c3d4e5f6a7b8a1b2c3d4e5f6a7b8", "b1b2c3d4e5f6a7b8a1b2c3d4e5f6a7b8"):
            await sink.put(
                image_id=image_id,
                image_bytes=b"jpeg",
                filename="img.jpg",
        ...
        assert patch_aioboto3.Session.return_value.client.call_count == 1

    async def test_aclose_closes_the_warm_client(self, fake_s3_client: _FakeS3Client) -> None:
        """``aclose`` exits the client context once and is safe to call when idle."""
        sink = _make_sink()
        await sink.aclose()  # never opened — no-op
        assert fake_s3_client.exit_count == 0

        await sink.put(
            image_id="c1b2c3d4e5f6a7b8a1b2c3d4e5f6a7b8",
            image_bytes=b"jpeg",
            filename="img.jpg",
            capture_metadata={},
            upload_metadata={},
        ...
        assert fake_s3_client.exit_count == 1
