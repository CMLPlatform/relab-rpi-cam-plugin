...
@pytest.fixture
def relay_state() -> RelayRuntimeState:
    """Fresh relay activity state for each test."""
...
class TestShouldBeRunning:
    ...
    def test_hibernate_disabled_means_always_on(self, pipeline: MagicMock, relay_state: RelayRuntimeState) -> None:
    ...
    def test_not_relay_enabled_means_hibernate(
        ...
        sleeper = PreviewSleeper(
            pipeline=pipeline, relay_state=relay_state, relay_enabled_getter=lambda: False, hibernate_after_s=300
        )
    ...
    def test_relay_connected_but_idle_past_threshold_means_hibernate(
        ...
        relay_state.mark_connected()
        monkeypatch.setattr(relay_state, "seconds_since_last_activity", lambda: 301.0)
        sleeper = PreviewSleeper(
            pipeline=pipeline, relay_state=relay_state, relay_enabled_getter=lambda: True, hibernate_after_s=300
        )
        assert sleeper.should_be_running() is False
    ...
    def test_stale_hls_activity_does_not_keep_encoder_awake(
        ...
        monkeypatch.setattr(relay_state, "seconds_since_last_hls_activity", lambda: 301.0)
        monkeypatch.setattr(relay_state, "seconds_since_last_activity", lambda: 301.0)
        relay_state.mark_connected()
        sleeper = PreviewSleeper(
            pipeline=pipeline, relay_state=relay_state, relay_enabled_getter=lambda: True, hibernate_after_s=300
