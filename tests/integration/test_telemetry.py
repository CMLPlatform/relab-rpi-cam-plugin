...
def _mock_psutil(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(telemetry_mod.psutil, "cpu_percent", lambda _interval=None: 7.5)
...
class TestTelemetryRoute:
    ...
    async def test_returns_200_and_schema(self, client: AsyncClient) -> None:
        ...
        data = resp.json()
        assert data["cpu_temp_c"] == 48.2
        assert data["cpu_percent"] == 7.5
        assert data["mem_percent"] == 31.0
        assert data["disk_percent"] == 25.0
        assert data["thermal_state"] == "normal"
