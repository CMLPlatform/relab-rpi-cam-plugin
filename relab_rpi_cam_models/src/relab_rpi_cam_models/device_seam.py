...
# ASCII-safe: a non-ASCII fingerprint would otherwise reach hmac.compare_digest
# and raise TypeError -> unauthenticated 500 on the backend. Confirmed against
# the Pi's own generator (`secrets.token_urlsafe(16)`, ~22 chars of
# [A-Za-z0-9_-]), which comfortably fits inside this pattern.
_FINGERPRINT_PATTERN = r"^[A-Za-z0-9._~-]{8,64}$"

# P-256 JWK coordinates are 32-byte big-endian integers, base64url-encoded
# without padding: always 43 characters. Bound loosely (42-64) rather than
# pinning the exact length in case of leading-zero trimming differences.
_JWK_COORDINATE_PATTERN = r"^[A-Za-z0-9_-]+$"
...
_DeviceFingerprint = Annotated[str, Field(min_length=8, max_length=64, pattern=_FINGERPRINT_PATTERN)]
...
PAIRING_TTL_CEILING_SECONDS = 600
"""Upper bound `PairingRegisterResponse.expires_in` will accept.

The backend's `PAIRING_TTL_SECONDS` must stay at or below this value, or its
own pairing-register responses will fail outbound validation."""
...
class DevicePublicKeyJWK(BaseModel):
    ...
    x: _JwkCoordinate
    y: _JwkCoordinate
    use: Annotated[str, Field(max_length=64)] | None = None
    kid: Annotated[str, Field(max_length=64)] | None = None
...
class PairingRegisterResponse(BaseModel):
    ...
    code: PairingCode
    expires_in: int = Field(gt=0, le=PAIRING_TTL_CEILING_SECONDS, description="Seconds until the pairing code expires.")
...
class RelayCommandEnvelope(BaseModel):
    ...
    path: Annotated[str, Field(min_length=1, max_length=2048, pattern=r"^[\x21-\x7e]+$")]
