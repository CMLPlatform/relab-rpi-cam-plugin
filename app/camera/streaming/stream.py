...
from app.camera.streaming.mediamtx_client import HIRES_RTSP_URL
...
def build_rtsp_ffmpeg_output(target_url: str) -> object:
    """Build an ``FfmpegOutput`` that RTSP-publishes an H264 stream to ``target_url``.

    Picamera2's ``FfmpegOutput`` runs a small ffmpeg subprocess that muxes the
    H264 stdin stream into the given output — a tiny H264→RTSP remuxer with no
    filters, no audio, no re-encoding. MediaMTX handles the rest (LL-HLS
    preview, YouTube egress, anything else). Shared by the hires stream and the
    always-on lores preview pipeline.
    """
...
def build_hires_rtsp_output() -> object:
    """Build the ``FfmpegOutput`` that publishes the main H264 encoder to MediaMTX cam-hires."""
    return build_rtsp_ffmpeg_output(HIRES_RTSP_URL)
